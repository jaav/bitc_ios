...
-(id)initWithNibNameAndItemwithoutMap:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil item:(Item *)item;
{
    self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil];
    if (self) {
        // Custom initialization
...
-(void)setDetailContent {
    ...
    if(isPad){
        ...
        int scrollHeight = 350 + textview.frame.size.height;
        
        if(scrollHeight > 536) {
            [scrollView setContentSize:CGSizeMake(320, scrollHeight)];
        }
        else{
            [scrollView setContentSize:CGSizeMake(320, 536)];
        }
    }
